...
async def copy_to_managers(message: types.Message):
    ...
    managers = await users.select_users_by_type(manager_user_type['id'])
    for manager in managers:
        try:
            await bot.copy_message(manager['telegram_id'], message.chat.id, message.message_id, "Сообщение от пользователя",
